...
int banda=4;
int canal=11;
int modo=0; // modo: 0=AM/FM, 1=USB, 2=LSB
int modoold=3; // 
bool mode=0; // mode: 0=canalizado, 1=banda corrida
/*
...
void setup() 
  ...
  display.display();
  pinMode(key_am,   INPUT_PULLUP);
  pinMode(key_usb,  INPUT_PULLUP);
  pinMode(key_lsb,  INPUT_PULLUP);
...
void loop() 
  ...
  if (digitalRead(key_usb)) modo=1;
  else if (digitalRead(key_lsb)) modo=2;
  else modo=0;  
  if (modoold!=modo)
    {
    generarfrecuencia();
...
void generarfrecuencia()
  ...
  switch(modo)
  {
    ...
    case 1:  si5351.set_freq((frec-fint+clarif+offset_ssb) * 100ULL, SI5351_CLK0); break; //modo USB
    case 2:  si5351.set_freq((frec-fint+clarif-offset_ssb) * 100ULL, SI5351_CLK0); break; //modo LSB
  }
