...
#define band15     9         //Este pin se usa para que el micro sepa que se selecciono la banda de 15
#define pinTRX     A3        //Este pin se usa para que el micro sepa si esta en TX o RX
#define meterRX    A0        //pin de entrada analogica del indicador de tension de entrada
#define meterTX    A1        //pin de entrada analogica del indicador de potencia de salida
#define meterVDC   A2        //pin de entrada analogica del indicador de recepcion
...
bool modo = 0;
bool save = 1;
int banda=0;                 //banda 1=60m, 2=40m, 3=20m, 4=15m
int count=0;
float volt=0;
ISR(PCINT2_vect) 
  {
...
void set_frequency(short dir) 
  ...
  if (encoder == 1) 
    {                         
    if (dir == 1) 
    {freq = freq + fstep; if (freq>=600000) freq=600000;}
    if (dir == -1) 
...
void setup() 
  ...
  EEPROM.get(0,freq);
  freqold = freq;
  if (digitalRead(band80) == HIGH && banda!=1) {freqbanda= 3400000;banda=1;modo=1;}
  if (digitalRead(band40) == HIGH && banda!=2) {freqbanda= 6900000;banda=2;modo=1;}
  if (digitalRead(band20) == HIGH && banda!=3) {freqbanda=13900000;banda=3;modo=0;}
  if (digitalRead(band15) == HIGH && banda!=4) {freqbanda=20900000;banda=3;modo=0;}
  tunegen();  
  }
...
void loop() 
  ...
  if (digitalRead(band15) == HIGH && banda!=4) 
    {
    banda=4;
    freqbanda= 20900000;
    delay(300);
...
void displayfreq() 
  ...
  volt=(float(analogRead(meterVDC))/1024.0)*15.66;
  display.print(volt);
  display.print("V");
