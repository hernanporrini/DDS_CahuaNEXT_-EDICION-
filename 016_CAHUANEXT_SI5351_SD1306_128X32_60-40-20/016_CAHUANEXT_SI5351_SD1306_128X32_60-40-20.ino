...
#define IF         1650000      //el valor de la frecuencia del filtro a cristal
#define tunestep   4         //Este pin es el pulsador central del rotary, se usa para cambiar el paso
#define band60     12        //Este pin se usa para que el micro sepa que se selecciono la banda de 60
...
void setup() 
  ...
  EEPROM.get(0,freq);
  freqold = freq;
  if (digitalRead(band60) == HIGH && banda!=1) {freqbanda= 4900000;banda=1;modo=1;}
  if (digitalRead(band40) == HIGH && banda!=2) {freqbanda= 6500000;banda=2;modo=1;}
  if (digitalRead(band20) == HIGH && banda!=3) {freqbanda=13500000;banda=3;modo=0;}
  tunegen();  
  }
...
void loop() 
  ...
  if (digitalRead(band60) == HIGH && banda!=1) 
    {
    banda=1;
    freqbanda=4900000;
    delay(300);
...
void displayfreq() 
  ...
  display.drawPixel(25, 30,WHITE);
  display.drawPixel(34, 30,WHITE);
  display.drawPixel(43, 30,WHITE);
  display.drawPixel(52, 30,WHITE);
  display.drawPixel(61, 30,WHITE);
  display.drawPixel(70, 30,WHITE);
  display.drawPixel(79, 30,WHITE);
  ...
  volt=(float(analogRead(meterVDC))/1024.00)*20.00;
  display.print(volt);
  display.print("V");
