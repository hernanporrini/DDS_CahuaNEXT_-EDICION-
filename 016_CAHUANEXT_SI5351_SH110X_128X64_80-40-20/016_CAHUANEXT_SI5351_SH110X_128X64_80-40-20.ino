...
#define meterRX    A3        //pin de entrada analogica del indicador de recepcion
...
void set_frequency(short dir) 
  ...
  if (encoder == 1) 
    {                         
    if (dir == 1) 
    {freq = freq + fstep; if (freq>=5000000) freq=5000000;}
    if (dir == -1) 
...
void setup() 
  ...
  EEPROM.get(0,freq);
  freqold = freq;
  if (digitalRead(band80) == LOW && banda!=1) {freqbanda= 3500000;banda=1;modo=1;}
  if (digitalRead(band40) == LOW && banda!=2) {freqbanda= 3500000;banda=2;modo=0;}
  if (digitalRead(band20) == LOW && banda!=3) {freqbanda= 8800000;banda=3;modo=1;}
  tunegen();  
  }
