...
#define tunestep   A0        //Este pin es el pulsador central del rotary, se usa para cambiar el paso
...
#define band20     10        //Este pin se usa para que el micro sepa que se selecciono la banda de 30
...
long cal = 127016;     //Si5351 factor de calibracion , ajustar exactamnente 10MHz. incrementando este valor se reduce la frecuencia generada y viceversa
byte encoder = 1;
byte stp=2;
bool save = 1;
bool modo = 1;
...
void setup() 
  ...
  lcd.setCursor(0, 0);
  lcd.print("CAHUANEXT - V4.0");
  lcd.setCursor(0, 1);
  lcd.print("#06 by H.Porrini");
  delay(5000);
  lcd.clear();  
  ...
  EEPROM.get(0,freq);
  freqold = freq;
  if (digitalRead(band80) == HIGH && banda!=1) {freqbanda= 3500000;banda=1;modo=1;}
  if (digitalRead(band40) == HIGH && banda!=2) {freqbanda= 6800000;banda=2;modo=1;}
  if (digitalRead(band20) == HIGH && banda!=3) {freqbanda=13800000;banda=3;modo=0;}
  tunegen();  
  }
...
void sgnalread() 
  {
  lcd.setCursor(0, 1); 
  controlTX=analogRead(A2);
  controlRX=analogRead(A3);
  if (controlTX > 50)
    {lcd.print("TX");smeter_signal = map(sqrt(analogRead(A2) * 16), 0, 128, 0, 80);}
  else if (controlRX > 50)
    {lcd.print("RX");smeter_signal = map(sqrt(analogRead(A3) * 16), 40, 128, 0, 100);}
